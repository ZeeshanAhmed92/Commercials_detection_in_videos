...
───────────────────────────────────
1. INTEGER HASH KEY  — SHA-1 string keys replaced with int64 bitpacked keys
   (same formula as S2).  DB TEXT values are converted to int on load.
   Integer dict lookup is ~5x faster than string lookup.

2. VECTORISED HASH GENERATION  — the O(N x FAN_VALUE) Python loop replaced
   with numpy broadcast.  Typical speedup: 10-20x for a 1-hour file.

3. FAST PEAK SORT  — np.argsort replaces Python sorted(zip(...)).

4. BINARY EROSION REMOVED  — redundant O(FxT) pass; amp_min_db already
   filters the noise floor.

5. SKIP REFINEMENT FOR HIGH-CONFIDENCE HITS  — detections with score >= 0.95
   skip fftconvolve (coarse boundaries are already tight at that confidence).

6. SKIP REFINEMENT FOR VERY-LOW-CONFIDENCE HITS  — detections below
   REFINE_MIN_SCORE skip cross-correlation and keep coarse boundaries,
   avoiding wasted xcorr on uncertain candidates.
...
try:
    from config import HASH_GEN_WORKERS
except ImportError:
    HASH_GEN_WORKERS = 4

try:
    from config import S3_MAX_FILE_WORKERS
except ImportError:
    S3_MAX_FILE_WORKERS = 6

# Score threshold above which we skip xcorr (coarse is already tight)
try:
    from config import REFINE_SKIP_HIGH_SCORE
except ImportError:
    REFINE_SKIP_HIGH_SCORE = 0.95

# Score threshold below which we also skip xcorr (not worth refining)
try:
    from config import REFINE_MIN_SCORE
except ImportError:
    REFINE_MIN_SCORE = 0.0   # 0 = refine everything (safe default)

# soundfile is 2-5x faster than librosa.load for raw WAV
try:
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HASH KEY PACKING  (must be identical to S2)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _load_audio(path: str, sr_target: int):
    if _USE_SOUNDFILE:
...
def _load_language(db_path, lang, ad_id_filter, cur):
    from S2_fingerprint_db import _table_names
    ...
    cur.execute(fp_query, params)
    for h_str, ad_id, t_ad in cur:
        # Convert TEXT key back to int for fast dict lookup
        try:
            h = int(h_str)
        except (ValueError, TypeError):
            h = h_str   # fallback for old SHA-1 rows
        partial_idx[h].append((ad_id, t_ad))
...
def compute_peaks(y: np.ndarray, sr: int) -> np.ndarray:
    """
    Return (N, 2) int32 array [freq_bin, time_frame] sorted by time.

    Changes vs original:
      - Returns numpy array (not list-of-tuples)
      - np.argsort replaces Python sorted()
      - binary_erosion removed
    """
    S    = np.abs(librosa.stft(y, n_fft=N_FFT, hop_length=HOP_LENGTH))
    S_db = librosa.amplitude_to_db(S, ref=np.max)
...
def generate_hashes(peaks, hop_length=HOP_LENGTH, sr=SR_EXPECTED, num_workers=1):
    """
    Vectorised fingerprint generation (replaces SHA-1 string loop).

    Accepts (N, 2) numpy array or legacy list-of-tuples.
    Returns [(hash_int, time_offset_seconds), ...].
    num_workers kept for API compatibility but vectorisation makes threading
    unnecessary — all work is done in numpy C code.
    """
...
def _accumulate_chunk(chunk, fp_index):
    local_acc = defaultdict(lambda: defaultdict(list))
    for h, t_mix in chunk:
        if h not in fp_index:
            continue
        for ad_id, t_ad in fp_index[h]:
...
def _merge_accumulators(partials):
...
def _xcorr_normalised(seg, template):
    if len(seg) < len(template) or len(template) < 1:
        return np.array([0.0]), 0.0
    corr = fftconvolve(seg, template[::-1], mode="valid")
    norm = (np.linalg.norm(seg) + 1e-10) * (np.linalg.norm(template) + 1e-10)
...
def local_refine_start(mixed_y, ad_y, sr, coarse_s, margin=LOCAL_MARGIN):
    i0  = max(0, int((coarse_s - margin) * sr))
    i1  = min(len(mixed_y), int((coarse_s + margin + min(len(ad_y) / sr, margin)) * sr))
    seg = mixed_y[i0:i1]
    prefix_len = min(len(ad_y), int(margin * sr * 1.5))
...
def local_refine_end(mixed_y, ad_y, sr, refined_s, coarse_e,
                     canonical_dur=None, margin=LOCAL_MARGIN):
    expected_end = (refined_s + canonical_dur) if canonical_dur else coarse_e
    i0  = max(0, int((coarse_e - margin) * sr))
    i1  = min(len(mixed_y), int((expected_end + margin) * sr))
    seg = mixed_y[i0:i1]
...
def refine_boundaries(mixed_y, ad_y, sr, coarse_s, coarse_e, canonical_dur=None):
    s2 = local_refine_start(mixed_y, ad_y, sr, coarse_s)
    e2 = local_refine_end(mixed_y, ad_y, sr, s2, coarse_e, canonical_dur)
    if e2 <= s2:
        return s2, coarse_e
...
def detect_for_file(
    ...
    ads_audio_folder,
    ad_audio_cache=None,
    num_hash_chunks=HASH_LOOKUP_CHUNKS,
    num_refine_workers=REFINE_WORKERS,
):
    print(f"  -> Scanning: {os.path.basename(mixed_path)}")

    y, sr0    = _load_audio(mixed_path, SR_EXPECTED)
    total_dur = librosa.get_duration(y=y, sr=sr0)

    # Peaks + vectorised hashes (no SHA-1, no Python loop)
    peaks  = compute_peaks(y, sr0)
    hashes = generate_hashes(peaks)
    if not hashes:
        print(f"  [WARN] No hashes generated for {os.path.basename(mixed_path)}")
    ...
    # Phase B: parallel boundary refinement
    needed_ads = {d["ad_id"] for d in merged}
    if ad_audio_cache is not None:
        ad_audio = {ad_id: ad_audio_cache.get(ad_id) for ad_id in needed_ads}
    else:
        ad_audio = {}
        for ad_id in needed_ads:
            ad_path = os.path.join(ads_audio_folder, f"{ad_id}.wav")
            ad_audio[ad_id] = _load_audio(ad_path, SR_EXPECTED)[0] if os.path.exists(ad_path) else None
    ...
    def _refine_one(det):
        ...
        # Skip xcorr for very high confidence — coarse boundaries are already tight
        if score >= REFINE_SKIP_HIGH_SCORE or ad_y is None:
            refined_s, refined_e = det["coarse_s"], det["coarse_e"]
        # Skip xcorr for very low confidence — not worth the compute
        elif REFINE_MIN_SCORE > 0 and score < REFINE_MIN_SCORE:
            refined_s, refined_e = det["coarse_s"], det["coarse_e"]
        else:
            refined_s, refined_e = refine_boundaries(
                y, ad_y, sr0, det["coarse_s"], det["coarse_e"], canonical_dur=ad_dur
        ...
        is_high_score = score >= FULL_SCORE_FLOOR
        typ = "Full" if (is_full_length and is_high_score) else "Partial"

        return {
            "ad_id":    ad_id,
            "start":    max(0.0, refined_s),
            "end":      min(total_dur, refined_e),
...
def detect_ads(
    ...
    # Pre-load all ad WAVs once and reuse across files
    ad_ids = set(ad_durations.keys())
    ad_audio_cache = {}
    if ad_ids:
        print(f"[S3] Pre-loading {len(ad_ids)} ad WAV(s) for reuse across files ...")
        for ad_id in ad_ids:
            ad_path = os.path.join(ads_audio_folder, f"{ad_id}.wav")
            ad_audio_cache[ad_id] = _load_audio(ad_path, SR_EXPECTED)[0] if os.path.exists(ad_path) else None
    ...
    workers = num_workers or min(S3_MAX_FILE_WORKERS, len(files), max(1, multiprocessing.cpu_count()))
